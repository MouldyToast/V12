...
class BasisAdapter:
    ...
    def get_adapted_basis(self, T: int) -> np.ndarray:
        ...
        if T not in self._basis_cache:
            ...
            # NOTE: QR orthogonalization REMOVED in fixed version.
            # When using correct control point projection, orthonormality
            # is not required because we project in control point space
            # where U_ref IS orthonormal, then use B-spline interpolation
            # only for reconstruction (not projection).

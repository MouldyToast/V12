"""
Singular Space Preprocessing - ANCHOR MODEL WITH RESIDUAL NORMALIZATION

This is the anchor-based model WITH per-dimension residual normalization.
The normalization is CRITICAL for diffusion models which start from N(0,1) noise.

KEY CHANGE FROM ORIGINAL:
=========================
After computing residuals = coefficient - anchor, we normalize them:
    normalized_residual = (residual - residual_mean) / residual_std

At generation time, DENORMALIZE before adding anchor:
    r = r_normalized * residual_std + residual_mean
    coefficients = anchor + r

Output Structure:
    processed_v3_anchor_norm/
...
# =============================================================================
# STEP 5.5: NORMALIZE RESIDUALS (CRITICAL FOR DIFFUSION!)
# =============================================================================
...
def normalize_residuals(residuals: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    Without normalization, the model struggles to learn the ~1000x scale-up
    from standard Gaussian to the actual residual distribution.
    ...
    Args:
        residuals: [N × K] raw residual array

    Returns:
        normalized: [N × K] normalized residuals (mean=0, std=1 per dim)
        residual_mean: [K] per-dimension means
        residual_std: [K] per-dimension standard deviations
    """
    print_section("STEP 5.5: Normalizing Residuals (CRITICAL FOR DIFFUSION)")

    K = residuals.shape[1]

    # Compute per-dimension statistics
    residual_mean = residuals.mean(axis=0)  # [K]
    residual_std = residuals.std(axis=0)    # [K]

    # Avoid division by zero for any constant dimensions
    ...
    print(f"  Per-dim std range: [{residuals.std(axis=0).min():.4f}, {residuals.std(axis=0).max():.4f}]")
    ...
    print(f"\nPer-dimension normalization (first 8):")
    for k in range(min(K, 8)):
        print(f"  r[{k}]: mean={residual_mean[k]:>10.4f}, std={residual_std[k]:>10.4f} -> normalized")
    if K > 8:
        print(f"  ... ({K - 8} more dimensions)")
...
def save_all_data(
    ...
    # Save residual normalization parameters (CRITICAL for generation!)
    ...
    full_config = {
        ...
        **config,
        ...
        'uses_residual_normalization': True,
...
"""
    ...
    # Step 6: Split data (using NORMALIZED residuals)
    splits = split_data(
    ...
    output_path = save_all_data(
        args.output, U_ref, T_ref, mean, group_anchors,
    ...
    print("\n" + "=" * 70)
    print("PREPROCESSING COMPLETE - ANCHOR MODEL WITH NORMALIZATION")
    print("=" * 70)
    ...
    print(f"  - train/val/test/:        Split data with NORMALIZED residuals!")
    ...
    print(f"  python generate_trajectory_v3_anchor.py --checkpoint checkpoints/best_model.pt --data {args.output}")

#!/usr/bin/env python3
"""
Trajectory Generation - ANCHOR MODEL WITH RESIDUAL NORMALIZATION

This script generates mouse trajectories using the anchor-based diffusion model
WITH residual normalization.

ANCHOR MODEL WITH NORMALIZATION:
================================
- Loads normalized residual parameters (residual_mean.npy, residual_std.npy)
- CRITICAL: Denormalizes residuals BEFORE adding to anchor
- Uses the original training script (train_singular_diffusion_v1.py)
...
Generation Process:
    ...
    5. DENORMALIZE residual: r = r_normalized * std + mean
    6. Add denormalized residual to anchor: c = anchor + r
    7. Reconstruct: trajectory = U_ref @ c
...
class TrajectoryGeneratorV3:
    ...
    def __init__(self, checkpoint_path: str, data_dir: str, device: str = 'cuda'):
        ...
        # Load residual normalization parameters (CRITICAL for anchor model!)
        ...
        if (self.data_dir / 'residual_mean.npy').exists():
            ...
            print(f"    Residual std range: [{self.residual_std.min():.4f}, {self.residual_std.max():.4f}]")
        ...
        print(f"  Mode: ANCHOR (with residual normalization)")
    ...
    def generate_single(
        ...
        r_0 = r_t.squeeze(0).cpu().numpy()  # [K]

        # DENORMALIZE residuals BEFORE adding anchor (CRITICAL!)
        ...
        coefficients = anchor.squeeze(0).cpu().numpy() + r_0  # [K]
    ...
    def generate_batch(
        ...
        residuals = r_t.cpu().numpy()  # [B, K]

        # DENORMALIZE residuals BEFORE adding anchor (CRITICAL!)
        if self.residual_mean is not None and self.residual_std is not None:
            residuals = residuals * self.residual_std + self.residual_mean

        # Coefficients = anchor + denormalized residual
        coefficients = anchors.cpu().numpy() + residuals  # [B, K]
...
def visualize_length_comparison(
    ...
    # Denormalize residuals before adding anchor
    r_0 = r_t.squeeze(0).cpu().numpy()
    if generator.residual_mean is not None and generator.residual_std is not None:
        r_0 = r_0 * generator.residual_std + generator.residual_mean

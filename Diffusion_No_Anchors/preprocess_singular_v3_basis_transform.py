...
Output Structure:
    ...
    ├── coeff_mean.npy         # [K] per-dim coefficient means (for denormalization)
    ├── coeff_std.npy          # [K] per-dim coefficient stds (for denormalization)
    ├── config.npy             # Configuration and metadata
    └── train/val/test/
        ├── coefficients.npy   # [N × K] - NORMALIZED coefficients (mean=0, std=1)
        ├── orientation_ids.npy
        ├── distance_ids.npy
...
DEFAULT_CONFIG = {
    # Output location
    'output_location': 'processed_v3/',
    'input_location': 'trajectories/',

    'version': 'v3_basis_transform',
    # Core parameters - NOTE: Aligned with paper's recommendations
    'K': 15,                     # Singular space dimensions (paper uses K=4)
    ...
    'max_distance_ratio': 2.0,  # actual_distance / ideal_distance threshold
    ...
    # Centering
    'center_data': False,        # Whether to subtract mean before SVD
}
...
DISTANCE_GROUPS = [
    {"name": "XSmall", "id": 0, "min": 0,   "max": 159},
    {"name": "Small",  "id": 1, "min": 159,  "max": 291},
    {"name": "Medium", "id": 2, "min": 291, "max": 423},
...
def load_trajectories(data_dir: Path, config: Dict) -> List[Dict]:
    ...
    Returns:
        List of trajectory dictionaries with keys:
            ...
            - actual_distance, orientation_id, distance_group_id
    """
...
def normalize_coefficients(
    ...
    """
    Normalize coefficients to zero-mean, unit-variance per dimension.

    This is CRITICAL for diffusion models which start from N(0,1) noise.
    Without normalization, the model can't learn the proper scale.

    Args:
        coefficients: [N × K] raw coefficients

    Returns:
        normalized: [N × K] normalized coefficients (mean=0, std=1 per dim)
        coeff_mean: [K] per-dimension means
        coeff_std: [K] per-dimension standard deviations
    """
    print_section("STEP 4: Normalizing Coefficients")

    K = coefficients.shape[1]

    # Compute per-dimension statistics
    coeff_mean = coefficients.mean(axis=0)  # [K]
    coeff_std = coefficients.std(axis=0)    # [K]

    # Avoid division by zero for constant dimensions
    coeff_std = np.maximum(coeff_std, 1e-8)

    # Normalize
    normalized = (coefficients - coeff_mean) / coeff_std

    # Report statistics
    print(f"  Raw coefficient statistics:")
    print(f"    Range: [{coefficients.min():.2f}, {coefficients.max():.2f}]")
    print(f"    Per-dim std range: [{coeff_std.min():.2f}, {coeff_std.max():.2f}]")

    print(f"\n  Normalized coefficient statistics:")
    ...
    print(f"\n  Per-dimension std (raw → normalized):")
    for k in range(min(K, 8)):
        print(f"    c[{k}]: {coeff_std[k]:>10.2f} → 1.00")
    if K > 8:
        print(f"    ... ({K - 8} more)")

    return normalized, coeff_mean, coeff_std


# =============================================================================
# STEP 5: SPLIT DATA (Previously Step 6 - anchors/residuals removed)
# =============================================================================
...
def split_data(
    ...
    Args:
        ...
        coefficients: [N × K] array - NORMALIZED coefficients for generation
    ...
    for name, idx in [('train', train_idx), ('val', val_idx), ('test', test_idx)]:
        splits[name] = {
            'indices': idx,
            'coefficients': coefficients[idx],  # Full coefficient space for generation
...
# =============================================================================
# STEP 6: SAVE DATA (Previously Step 7 - anchors removed)
# =============================================================================
...
def save_all_data(
    ...
    Output structure:
        ...
        ├── coeff_mean.npy         # Per-dim coefficient means [K]
        ├── coeff_std.npy          # Per-dim coefficient stds [K]
        ├── config.npy             # Full configuration
        └── train/val/test/
            ├── coefficients.npy   # NORMALIZED coefficients for generation
            ├── orientation_ids.npy
            ├── distance_ids.npy
    ...
    Args:
        ...
        coeff_mean: Per-dimension coefficient means
        coeff_std: Per-dimension coefficient stds
    ...
    """
    print_section("STEP 6: Saving Data")
    ...
    # Save coefficient normalization parameters (CRITICAL for generation!)
    np.save(output_path / 'coeff_mean.npy', coeff_mean)
    np.save(output_path / 'coeff_std.npy', coeff_std)
    print(f"  Saved: coeff_mean.npy {coeff_mean.shape}")
    print(f"  Saved: coeff_std.npy {coeff_std.shape}")
...
# =============================================================================
# STEP 7: VERIFICATION (Previously Step 8)
# =============================================================================
...
"""
    ...
    # Print header
    print("=" * 70)
    print("SINGULAR SPACE PREPROCESSING - VERSION 3 (GENERATION MODE)")
    print("=" * 70)
    print(f"\nInput:  {args.input}")
    ...
    # Step 4: Normalize coefficients (CRITICAL for diffusion!)
    normalized_coefficients, coeff_mean, coeff_std = normalize_coefficients(coefficients)

    # Step 5: Split data (using normalized coefficients)
    splits = split_data(trajectories, normalized_coefficients, config)
    ...
    output_path = save_all_data(
        args.output, U_ref, T_ref, mean, coeff_mean, coeff_std, splits, config, all_stats
    )

    # Analyze and save length distributions
    length_stats = analyze_length_distributions(trajectories, output_path)
    ...
    print("\n" + "=" * 70)
    print("PREPROCESSING COMPLETE - VERSION 3 (GENERATION MODE)")
    print("=" * 70)
    ...
    print(f"  - coeff_mean.npy:         Per-dim means for denormalization [{config['K']}]")
    print(f"  - coeff_std.npy:          Per-dim stds for denormalization [{config['K']}]")
    print(f"  - length_distributions.npy: Length stats for generation")
    print(f"  - config.npy:             Full configuration")
    print(f"  - train/val/test/:        Split data with NORMALIZED coefficients")

    print(f"\nStatistics:")
    print(f"  - Trajectories: {len(trajectories)}")
    print(f"  - Variance explained: {basis_stats['variance_explained']*100:.2f}%")
    print(f"  - Raw coeff std range: [{coeff_std.min():.2f}, {coeff_std.max():.2f}]")
    print(f"  - Normalized coeff std: 1.0 (all dimensions)")

    print(f"\nNOTE: Coefficients are NORMALIZED (mean=0, std=1 per dimension).")
    print(f"      Generation script must DENORMALIZE using coeff_mean and coeff_std.")
